...
const char* THDrawSmooth::SHADER_NAME_POSITION_LENGTH_TEXTURE_COLOR_SMOOTH = "ShaderPositionColorLengthTextureSmooth";

THDrawSmooth::THDrawSmooth() : DrawNode() {}

THDrawSmooth::~THDrawSmooth() {
    
}

bool THDrawSmooth::init() {
    CCLOG("THDrawSmooth::init");
    ...
    setGLProgramState(
        GLProgramState::getOrCreateWithGLProgramName(THDrawSmooth::SHADER_NAME_POSITION_LENGTH_TEXTURE_COLOR_SMOOTH));
...
void THDrawSmooth::drawCircle(const Vec2& center, float radius, const Color4B& color, float line_width, int segments) {
    Vec2 a(0,0);
...
void THDrawSmooth::drawFillCircle(const Vec2& center, float radius, const Color4B color, int segments) {
    Vec2 a(0,0), b(0,0);
...
void THDrawSmooth::drawFillTriangle(const Vec2& p0, const Vec2& p1, const Vec2& p2, const Color4B& color) {
    // using cocos2d draw, fix our own
    drawTriangle(p0,p1,p2,Color4F(color));
}

void THDrawSmooth::drawFillTriangleB(const Vec2& a, const Vec2& b, const Vec2& c, const Color4B& color) {
...
void THDrawSmooth::drawRect(const Vec2& start, const Vec2& end, const Color4B& color, float line_width) {
    std::vector<Vec2> path;
...
void THDrawSmooth::drawFillRect(const Vec2& start, const Vec2& end, const Color4B &color) {
    drawTriangle(start, end, Vec2(start.x, end.y), Color4F(color));
    drawTriangle(start, end, Vec2(end.x, start.y), Color4F(color));
}

void THDrawSmooth::drawLine(const Vec2& start, const Vec2& end, const Color4B &color, float width) {
    std::vector<Vec2> path;
    path.push_back(start);
    path.push_back(end);
...
void THDrawSmooth::drawLinePath(const std::vector<Vec2>& path, float width, const Color4B& color, bool round_ends, bool close_path) {
...
void THDrawSmooth::drawLinePath(const std::vector<Vec2>& path, float width, const Color4B& color, bool round_ends) {
    drawLinePath(path, width, color, round_ends, false);
}

void THDrawSmooth::cornerDraw(const Vec2& a, const Vec2& b, const Vec2& c, const Color4B& color,
...
/*
void THDrawSmooth::drawPolygon(const Vec2 *verts, int count, const Color4B &fillColor, float borderWidth, const Color4B &borderColor)
{
...
void THDrawSmooth::drawCommand(const DrawCommand& command) {
    switch(command.type) {
        case DCT_NOTHING:
            return;
        default:
...
void THDrawSmooth::drawCommandQueue(const std::vector<DrawCommand>& command) {
...
void THDrawSmooth::draw(Renderer *renderer, const Mat4 &transform, uint32_t flags)
{
    if(_bufferCount)
    {
        _customCommand.init(_globalZOrder, transform, flags);
        _customCommand.func = CC_CALLBACK_0(THDrawSmooth::onDraw, this, transform, flags);
    ...
    if(_bufferCountGLPoint)
    {
        ...
        _customCommandGLPoint.func = CC_CALLBACK_0(THDrawSmooth::onDrawGLPoint, this, transform, flags);
    ...
    if(_bufferCountGLLine)
    {
        ...
        _customCommandGLLine.func = CC_CALLBACK_0(THDrawSmooth::onDrawGLLine, this, transform, flags);
        renderer->addCommand(&_customCommandGLLine);
    }
}

void THDrawSmooth::onDraw(const Mat4 &transform, uint32_t flags)
{
...
void THDrawSmooth::onDrawGLLine(const Mat4 &transform, uint32_t flags)
{
    auto glProgram = GLProgramCache::getInstance()->getGLProgram(THDrawSmooth::SHADER_NAME_POSITION_LENGTH_TEXTURE_COLOR_SMOOTH);
    glProgram->use();
    glProgram->setUniformsForBuiltins(transform);
...
void THDrawSmooth::onDrawGLPoint(const Mat4 &transform, uint32_t flags)
{
